...
export function HomeBudget() {
  ...
  const nudgeClass =
    "web-hit inline-flex size-11 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700 transition-all hover:border-ink-900 hover:bg-brand-dark hover:text-white active:scale-95 shadow-xs cursor-pointer";
      ...
      <div className="mt-14 grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] lg:gap-16 items-start">
        ...
        <div className="relative rounded-[24px] border border-slate-200/90 bg-white p-7 sm:p-9 shadow-[0_20px_50px_rgba(21,25,54,0.06),0_1px_3px_rgba(0,0,0,0.04)] transition-all">
          <div className="flex items-center justify-between">
            <label
              htmlFor={budgetId}
              className="font-mono text-web-micro font-medium uppercase tracking-[0.16em] text-slate-500"
            >
              Monthly budget (KES)
            </label>
          ...
          <div className="mt-3.5 flex items-center gap-3">
            ...
            <div className="relative min-w-0 flex-1">
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-center font-mono text-2xl sm:text-3xl font-medium tracking-tight text-ink-900 shadow-inner transition-all focus:border-ink-900 focus:bg-white focus:outline-none"
              />
            </div>
          ...
          <div className="mt-5">
            <input
              ...
              className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-100 accent-ink-900 focus:outline-none"
            />
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            {BUDGET_PRESETS.map((preset) => (
              <button
                ...
                className={cn(
                  "web-hit rounded-full px-3 py-1 font-mono text-web-micro font-medium transition-all cursor-pointer",
                  budget === preset
          ...
          {/* Bedrooms Selector */}
          <div className="mt-8">
            <label className="font-mono text-web-micro font-medium uppercase tracking-[0.16em] text-slate-500">
              Select Bedrooms
            </label>
              ...
              {([1, 2, 3] as const).map((count) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={cn(
                      "flex items-center justify-center gap-2 rounded-2xl py-3 text-xs sm:text-web-xs font-medium font-mono uppercase tracking-wider transition-all cursor-pointer",
                      active
                        ? "bg-brand-dark text-white shadow-sm ring-2 ring-ink-900 ring-offset-2"
                        : "border border-slate-200/90 bg-slate-50/80 text-slate-700 hover:bg-slate-100 hover:border-slate-300"
                    )}
                  >
                    {active && <CheckIcon size={14} stroke={2.5} className="text-brand-yellow" />}
          ...
          <div aria-live="polite" className="mt-8 flex items-center border-t border-slate-100 pt-6">
            <div className="flex items-center gap-3">
              <span className="font-mono text-3xl sm:text-4xl font-medium tracking-tight text-ink-900">
                {areaCount}
              </span>
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-3">
            <p className="font-mono text-web-micro font-medium uppercase tracking-[0.16em] text-slate-500">
              ...
              {matches.map((match) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="font-editorial text-2xl font-medium text-ink-900 transition-colors group-hover:text-blue-700">
                      {match.name}
                    </p>
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      <span className="text-web-xs text-slate-500">
                        {match.note}
                      </span>
                      <span className="text-slate-300">·</span>
                      <span
                        className={cn(
                          "inline-flex items-center rounded-full px-2.5 py-0.5 font-mono text-web-micro font-medium",
                          match.isAtCeiling
                  ...
                  <div className="shrink-0 text-right">
                    <p className="font-mono text-xl font-medium text-ink-900">
                      {match.typical}
                    </p>
                    <span className="font-mono text-web-nano text-slate-400 uppercase">
                      / month
                    </span>
                  </div>
                </li>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-4">
            ...
            <Link
              ...
              className="web-control web-hit inline-flex items-center rounded-full border border-slate-300 bg-white px-7 py-3 text-xs font-medium uppercase tracking-[0.12em] text-ink-900 transition-all hover:border-slate-400 hover:bg-slate-50"
            >
