      ...
      <Container>
        <div className="grid gap-12 lg:grid-cols-12 lg:items-center xl:gap-16">
          ...
          <div data-reveal className="lg:col-span-6 xl:col-span-7">
            ...
            <h2
              id="story-heading"
              className="font-editorial text-3xl sm:text-4xl lg:text-[44px] font-medium leading-[1.12] text-[#151936] tracking-tight"
            >
              {ABOUT_STORY.title}
            </h2>

            {/* Lead Narrative Statement */}
            <p className="mt-5 text-[17.5px] sm:text-[19px] leading-relaxed text-[#151936] font-normal border-l-2 border-brand-yellow pl-4 sm:pl-5">
              {ABOUT_STORY.paragraphs[0]}
            </p>

            {/* Supporting Story Paragraph */}
            <p className="mt-4 text-[15.5px] sm:text-base leading-relaxed text-slate-600 font-normal">
              ...
              {STORY_PILLARS.map((pillar) => {
                ...
                return (
                  <div key={pillar.title} className="group flex flex-col space-y-2.5">
                    <span className="flex size-8.5 shrink-0 items-center justify-center rounded-lg bg-surface-1 border border-line-soft text-[#151936] shadow-2xs transition-transform duration-300 group-hover:scale-105">
                      <IconComponent size={16} stroke={WEB_ICON_STROKE} />
                    </span>
                    <h3 className="font-editorial text-[17px] font-medium leading-snug text-[#151936]">
                      {pillar.title}
                    </h3>
            ...
            <div className="mt-7 flex items-start gap-2.5 border-t border-line-soft pt-4 font-mono text-xs text-slate-500 leading-relaxed">
              ...
              <p>
                We work across <span className="font-medium text-slate-700">20 areas</span>, from Kilimani and Lavington to Tatu City, Nyali, and upcountry acreage in Nyeri and Elgeyo Marakwet. Where we cannot service a property properly, we say so.
              </p>
            </div>
          </div>
          ...
          <div data-reveal data-reveal-x="24" className="lg:col-span-6 xl:col-span-5 flex flex-col gap-6">
            ...
            <div className="group relative aspect-[16/11] sm:aspect-[4/3] w-full overflow-hidden rounded-2xl border border-line bg-slate-900 shadow-md">
              <Image
                src="https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?fm=jpg&fit=crop&crop=entropy&ar=4:3&w=1400&q=85"
              ...
              <div
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#090d1f]/80 via-transparent to-transparent"
              />

              {/* Floating Top Badge */}
              <div className="absolute top-4 left-4 z-10 inline-flex items-center gap-1.5 rounded-full bg-black/60 backdrop-blur-md px-3.5 py-1 font-mono text-[11px] font-medium text-white border border-white/20 shadow-sm">
            ...
            <dl data-reveal-group className="grid grid-cols-3 divide-x divide-line pt-3 border-t border-line-soft">
              {figures.map((figure, idx) => (
                ...
                >
                  <dd className="font-mono text-2xl sm:text-[32px] font-semibold tracking-tight text-[#151936] leading-none">
                    {figure.value}
                  </dd>
                  <dt className="font-mono mt-2 text-[10.5px] uppercase tracking-[0.15em] text-slate-400 font-medium leading-tight">
                    {figure.label}
                  </dt>
                </div>
              ))}
